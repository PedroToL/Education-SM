McCrary_Test <- function (runvar, cutpoint, bin = NULL, bw = NULL, verbose = FALSE, 
          plot = TRUE, ext.out = FALSE, htest = FALSE) 
{
  ...
  if (plot) {
    ...
    plot(d.l$cellmp, d.l$est, lty = 1, lwd = 2, col = "black", 
         type = "l", xlim = c(pmin, pmax), ylim = c(0, 0.08), xlab = NA, ylab = NA, 
         main = NA)
...
kernelwts <- rdd::kernelwts()
McCrary_Test(df$year_centered)
title(xlab = "Running Variable Centered at 1981", ylab = "Density")
